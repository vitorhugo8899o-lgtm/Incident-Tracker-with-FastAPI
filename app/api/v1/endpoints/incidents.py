...
async def incident_create(
    current_user: CurrentUser, db: DBSession, incident: IncidentCreate
):
...
async def user_delete_incident(
    user: CurrentUser, db: DBSession, id_incident: int
):
...
@router_incident.get(
    '/incidents/history/{id_incident}', status_code=HTTPStatus.OK
)
